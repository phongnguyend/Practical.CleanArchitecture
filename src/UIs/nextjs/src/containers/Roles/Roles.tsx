...
export default function Roles({ mode, id }: { mode: "list" | "view"; id?: string }) {
  ...
  const load = useCallback(async () => {
    ...
    } catch (err) { setError("Unable to load roles. Please try again."); }
  }, [id, mode]);
  ...
  return <div className="card mt-2">
    ...
    <div className="card-body">
      ...
      {mode === "list" && <div className="table-responsive"><table className="table"><thead><tr><th>Name</th><th>Actions</th></tr></thead><tbody>
        {roles.map(item => <tr key={item.id}><td><Link href={`/roles/${item.id}`}><ActionIcon action="view" />{item.name}</Link></td><td>
    ...
    {mode !== "list" && <div className="card-footer"><Link className="btn btn-outline-secondary" href="/roles"><ActionIcon action="back" />Back</Link></div>}
